  ...
  return (
    <div className="min-h-screen pt-32 px-6 md:px-12 max-w-7xl mx-auto w-full pb-32 font-sans bg-brand-bg text-brand-text">
      <header className="mb-12 text-center flex flex-col items-center">
        ...
        <h1 className="text-5xl md:text-7xl font-serif text-brand-text mb-6">The Reading Room</h1>
        ...
        <div className="flex bg-brand-card/60 p-2 rounded-full border border-brand-border/30 overflow-x-auto w-full max-w-md mx-auto no-scrollbar relative z-10 mb-8">
          <button 
            onClick={() => setFeedSource("all")}
            className={`flex-1 text-[10px] uppercase tracking-widest px-6 py-3 rounded-full transition-all duration-500 font-bold whitespace-nowrap ${feedSource === "all" ? "bg-brand-text text-brand-bg" : "text-brand-soft/70 hover:text-brand-text"}`}
          >
            All Posts
          </button>
          <button 
            onClick={() => setFeedSource("following")}
            className={`flex-1 text-[10px] uppercase tracking-widest px-6 py-3 rounded-full transition-all duration-500 font-bold whitespace-nowrap ${feedSource === "following" ? "bg-brand-text text-brand-bg" : "text-brand-soft/70 hover:text-brand-text"}`}
          >
            Following
          </button>
          {currentUserId && (
            <button 
              ...
              className={`flex-1 text-[10px] uppercase tracking-widest px-6 py-3 rounded-full transition-all duration-500 font-bold whitespace-nowrap ${feedSource === "me" ? "bg-brand-text text-brand-bg" : "text-brand-soft/70 hover:text-brand-text"}`}
            >
              My Posts
            </button>
          )}
        ...
        <div className="w-full flex flex-col md:flex-row items-center justify-between gap-6 border-b border-brand-border/30 pb-6">
          {/* Categories */}
          <div className="flex gap-2 overflow-x-auto w-full md:w-auto pb-2 md:pb-0 scrollbar-hide">
            {categories.map(cat => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-[10px] uppercase tracking-widest transition-all whitespace-nowrap border ${categoryFilter === cat.id ? 'bg-brand-accent/15 border-brand-accent text-brand-accent font-bold' : 'bg-transparent border-brand-border/40 text-brand-soft hover:text-brand-text'}`}
              >
                {cat.label}
              </button>
            ))}
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            <select 
              ...
              className="bg-brand-card border border-brand-border/40 text-brand-text text-xs py-2 px-4 rounded-full outline-none focus:border-brand-accent appearance-none cursor-pointer pr-8 relative"
          ...
          displayedProfiles && displayedProfiles.length > 0 ? (
            displayedProfiles.map((profile) => (
              <article key={profile.id} className="break-inside-avoid bg-brand-card border border-brand-border/30 p-8 rounded-[2rem] hover:border-brand-accent/50 transition-colors duration-500 group relative overflow-hidden flex flex-col min-h-[250px] items-center text-center animate-fade-in-up">
                ...
                {profile.avatar_url ? (
                  <Image src={profile.avatar_url} alt={profile.username} width={80} height={80} className="rounded-full border border-brand-border/20 mb-4" />
                ) : (
                  <div className="w-20 h-20 rounded-full bg-brand-bg/50 border border-brand-border/20 flex items-center justify-center text-2xl font-bold text-brand-text mb-4">
                    {profile.display_name?.charAt(0) || profile.username?.charAt(0) || '?'}
                  </div>
                )}

                <h2 className="text-2xl font-serif text-brand-text mb-1 group-hover:text-brand-accent transition-colors relative z-10">
                  {profile.display_name || profile.username || 'Unknown'}
                </h2>
                <span className="text-[10px] text-brand-soft/60 uppercase tracking-widest block mb-4 relative z-10">@{profile.username}</span>
                ...
                {profile.is_premium && (
                  <span className="text-[9px] uppercase tracking-widest text-brand-accent bg-brand-accent/10 border border-brand-accent/25 px-3 py-1 rounded-full mb-4 inline-block relative z-10">
                    Guardian 🌿
                  </span>
                )}

                <Link href={`/room/${profile.username}`} className="mt-auto relative z-10 bg-brand-text text-brand-bg px-6 py-2 rounded-full text-[10px] uppercase tracking-widest font-bold hover:scale-105 transition-transform">
                  Enter Room
                </Link>
              </article>
            ))
              ...
              <article key={post.id} className="break-inside-avoid bg-brand-card border border-brand-border/30 p-8 rounded-[2rem] hover:border-brand-accent/50 transition-colors duration-500 group relative overflow-hidden flex flex-col min-h-[250px] animate-fade-in-up">
                ...
                <div className="flex items-center gap-3 mb-6 relative z-10 shrink-0">
                  <Link href={`/room/${post.profiles?.username}`} className="flex items-center gap-3 hover:opacity-80 transition-opacity z-20">
                    ...
                    <div>
                      <span className="text-sm text-brand-text font-bold block leading-tight">
                        {post.profiles?.display_name || post.profiles?.username || 'Unknown'}
                      </span>
                      <span className="text-[10px] text-brand-soft/60 uppercase tracking-widest">@{post.profiles?.username || 'unknown'}</span>
                    </div>
                  </Link>
                </div>
                ...
                <Link href={`/room/${post.profiles?.username}/${post.slug || post.id}`} className="block group-hover:opacity-90 transition-opacity relative z-10 flex-grow">
                  ...
                  {post.title && (
                    <h2 className="text-2xl font-serif text-brand-text mb-4 leading-snug group-hover:text-brand-accent transition-colors">
                      {post.title}
                    </h2>
                  )}
                ...
                <div className="pt-6 border-t border-brand-border/30 flex items-center justify-between relative z-10 shrink-0 mt-auto">
