    ...
    <div className="relative min-h-screen w-full font-sans selection:bg-brand-accent selection:text-white overflow-hidden bg-brand-bg text-brand-text">
      ...
      <section className="relative min-h-[85vh] md:min-h-[90vh] flex flex-col justify-center items-center px-4 md:px-6 text-center">
        <div className="z-10 flex flex-col items-center max-w-4xl mt-16 md:mt-12">
          <p className="text-xs tracking-[0.3em] uppercase mb-6 flex items-center gap-3 text-brand-soft animate-fade-in-up" style={{ animationDelay: '0s' }}>
          ...
          <p className="text-base md:text-xl max-w-2xl leading-relaxed mb-8 font-serif italic px-4 text-brand-soft animate-fade-in-up" style={{ animationDelay: '0.5s' }}>
            A digital sanctuary for overthinkers. Relieve stress with clinically-backed tools, share anonymous reflections, and read calming guides written by minds like yours.
          </p>

          <div className="flex flex-col items-center gap-3 w-full px-4">
            <Link href="/onboarding" className="w-full sm:w-auto px-8 py-4 rounded-full text-xs tracking-widest uppercase font-bold hover:scale-105 transition-transform shadow-lg text-center bg-brand-text text-brand-bg">
              Explore Calming Tools & Writings
            </Link>
            <div className="flex items-center gap-4">
              <Link href="/reading-room" className="text-xs uppercase tracking-widest transition-colors text-brand-soft hover:text-brand-text">
                Browse Essays
              </Link>
              <span className="text-brand-border">·</span>
              <Link href="/start" className="text-xs uppercase tracking-widest font-bold transition-colors text-brand-accent hover:text-brand-text">
                New here? Start here →
              </Link>
            </div>
          </div>
          ...
          {stats.members > 0 && (
            <p className="mt-8 text-xs uppercase tracking-widest text-brand-soft">
      ...
      <section className="relative w-full py-3 overflow-hidden z-20 border-t border-b border-brand-border bg-brand-card">
        <div className="flex whitespace-nowrap animate-marquee gap-12 text-xs font-sans tracking-widest uppercase text-brand-soft">
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-6 py-16 md:py-24">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-8 gap-3">
          <div>
            <span className="text-xs uppercase tracking-widest mb-2 block font-bold text-brand-accent">Latest from the sanctuary</span>
            <h2 className="text-3xl md:text-4xl font-serif text-brand-text">Calming Essays & Guides</h2>
          </div>
          <Link href="/reading-room" className="text-xs uppercase tracking-widest pb-1 font-bold shrink-0 text-brand-accent border-b border-brand-accent hover:text-brand-text">
            View All Essays →
          </Link>
        </div>
          ...
          {displayPosts.map((post: any) => (
            <Link key={post.id} href={`/room/${post.profiles?.username || 'unknown'}/${post.slug || post.id}`}
              ...
              <div className="flex items-center justify-between mb-3">
                <span className="text-xs uppercase tracking-widest font-bold px-2.5 py-1 rounded-full text-brand-accent bg-brand-accent/10 border border-brand-accent/20">
                  {categoryLabels[post.type] || post.type}
                </span>
                <span className="text-xs text-brand-soft">🕯️ {post.candle_count || 0}</span>
              </div>
              ...
              <div className="flex items-center gap-2 mt-auto pt-3 border-t border-brand-border/40">
                <div className="w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold bg-brand-accent/20 text-brand-text">
                  {post.profiles?.display_name?.charAt(0) || "S"}
                </div>
                <span className="text-xs uppercase tracking-widest text-brand-soft">@{post.profiles?.username}</span>
              </div>
            </Link>
          ))}
        </div>
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-6 py-8 md:py-12 grid grid-cols-1 md:grid-cols-3 gap-4">
        <Link href="/toolkit" className="md:col-span-2 p-6 md:p-10 rounded-2xl flex flex-col justify-between group transition-colors bg-brand-card border border-brand-border hover:border-brand-accent/50">
          <div className="mb-6">
            <span className="text-xs uppercase tracking-widest mb-3 block font-bold text-brand-accent">Interactive Toolkit</span>
            <h2 className="text-2xl md:text-4xl font-serif mb-3 text-brand-text">Clinical Tools for Mental Calmness</h2>
            <p className="text-sm max-w-md text-brand-soft">20 interactive tools for worry, panic, ADHD & overthinking — grounded in CBT, DBT & mindfulness research.</p>
          </div>
        ...
        <Link href="/pilgrim" className="p-6 md:p-10 rounded-2xl flex flex-col group transition-colors bg-brand-card border border-brand-border hover:border-brand-accent/50">
          <span className="text-xs uppercase tracking-widest mb-3 block font-bold text-brand-accent">Community Wall</span>
          <h2 className="text-2xl font-serif mb-2 text-brand-text">Anonymous Pilgrim Reflections</h2>
          <p className="text-sm mb-4 text-brand-soft">Real anonymous thoughts from people who needed this space.</p>
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-6 py-12 md:py-20">
        <div className="text-center mb-8">
          <span className="text-xs uppercase tracking-widest mb-2 block font-bold text-brand-accent">Kind Words</span>
          <h2 className="text-3xl md:text-4xl font-serif text-brand-text">Real Stories from Our Community</h2>
        </div>
          ...
          {testimonials.map((t, i) => (
            <div key={i} className="p-6 md:p-8 rounded-2xl bg-brand-card border border-brand-border">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs uppercase tracking-widest px-2 py-1 rounded-full text-brand-soft bg-brand-bg border border-brand-border/40">
                  {t.source}
                </span>
              </div>
            </div>
      ...
      <section className="max-w-4xl mx-auto px-4 md:px-6 py-12 md:py-20">
        <div className="p-6 md:p-10 rounded-2xl flex flex-col md:flex-row items-center gap-6 md:gap-10 bg-brand-card border border-brand-border">
          ...
          <div className="text-center md:text-left">
            <span className="text-xs uppercase tracking-widest mb-2 block font-bold text-brand-accent">The human behind this</span>
            ...
            <Link href="/about" className="text-xs uppercase tracking-widest font-bold transition-colors text-brand-accent border-b border-brand-accent hover:text-brand-text">
              Read my full story →
            </Link>
          </div>
        </div>
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-6 py-12 md:py-16">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-8 gap-3">
          <div>
            <span className="text-xs uppercase tracking-widest mb-2 block font-bold text-brand-accent">Shop</span>
          ...
          <Link href="/store" className="text-xs uppercase tracking-widest pb-1 font-bold shrink-0 text-brand-accent border-b border-brand-accent hover:text-brand-text">Browse All Products →</Link>
        </div>
            ...
            return (
              <Link key={idx} href={product.url}
                ...
                <div className="h-28 md:h-36 flex items-center justify-center relative transition-colors bg-brand-bg border-b border-brand-border">
                  ...
                  <span className="absolute top-2 left-2 text-xs uppercase tracking-widest px-2 py-0.5 rounded-full text-brand-soft bg-brand-bg border border-brand-border/40">
                    {product.tag}
                  </span>
                  <span className={`absolute top-2 right-2 text-xs font-bold px-2 py-0.5 rounded-full ${isFree ? "text-green-500 bg-green-500/10 border border-green-500/25" : "text-brand-accent bg-brand-accent/10 border border-brand-accent/25"}`}>
                    {product.price}
                  </span>
                </div>
                <div className="p-4 flex flex-col flex-1">
                  ...
                  <div className="mt-3 pt-3 border-t border-brand-border/40">
                    <span className={`text-xs uppercase tracking-widest font-bold ${isFree ? "text-green-500" : "text-brand-accent group-hover:text-brand-text"}`}>
      ...
      <section className="max-w-4xl mx-auto px-4 md:px-6 py-12 md:py-20">
        <div className="p-8 md:p-12 rounded-2xl text-center relative overflow-hidden bg-brand-card border border-brand-border shadow-sm">
          <div className="relative z-10">
            ...
            <p className="text-sm font-bold mb-6 text-brand-accent">
            ...
            {subStatus === "success" ? (
              ...
              <form onSubmit={handleSubscribe} className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
                ...
                <button type="submit" disabled={subStatus === "loading"}
                  className="px-6 py-3 rounded-full text-xs uppercase tracking-widest font-bold hover:scale-105 transition-transform disabled:opacity-50 shrink-0 bg-brand-text text-brand-bg">
            ...
            <p className="text-xs mt-4 uppercase tracking-widest text-brand-soft">
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-6 py-12 md:py-16">
        <div className="text-center mb-8">
          <span className="text-xs uppercase tracking-widest mb-2 block font-bold text-brand-accent">Explore</span>
        ...
        <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
          {exploreRooms.map((room) => (
            <Link key={room.path} href={room.path}
              ...
              <p className="text-xs uppercase tracking-widest mt-1 hidden md:block text-brand-soft">{room.desc}</p>
            </Link>
          ))}
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-6 py-12">
        <div className="rounded-2xl p-8 md:p-14 text-center flex flex-col items-center bg-brand-card border border-brand-border">
          ...
          <h2 className="text-2xl md:text-3xl font-serif mb-3 text-brand-text">Share your quiet voice with the world.</h2>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <Link href="/onboarding" className="px-6 py-3 rounded-full text-xs tracking-widest uppercase font-bold hover:scale-105 transition-transform text-center bg-brand-text text-brand-bg">Start Writing</Link>
            <Link href="/dashboard" className="px-6 py-3 rounded-full text-xs tracking-widest uppercase text-center transition-all text-brand-soft border border-brand-border hover:text-brand-text hover:border-brand-accent">Dashboard</Link>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-5xl mx-auto px-4 md:px-6 py-12 relative">
        ...
        <div className="relative rounded-2xl md:rounded-[3rem] overflow-hidden p-[1px] animate-pulse-glow">
          <div className="rounded-2xl md:rounded-[3rem] p-8 md:p-16 text-center relative z-10 flex flex-col items-center bg-brand-bg border border-brand-border/40 shadow-xl">
            ...
            <h2 className="text-3xl md:text-5xl font-serif mb-4 text-brand-text">Become a Sanctuary Guardian.</h2>
            ...
            <Link href="/sanctuary-pass" className="text-white px-8 py-4 rounded-full text-xs tracking-widest uppercase font-bold hover:scale-105 transition-transform bg-brand-accent shadow-[0_0_30px_rgba(201,164,106,0.35)]">
              View Sanctuary Pass
            </Link>
          </div>
        </div>
      ...
      <section className="py-16 text-center px-4 border-t border-brand-border bg-brand-bg">
        <p className="uppercase tracking-[0.3em] text-xs mb-6 text-brand-soft">The Quietly Humans Promise</p>
