...
export default async function PostPage({ params }: Props) {
  ...
  return (
    <div className={`min-h-screen pt-32 px-6 md:px-12 w-full pb-32 font-serif relative ${themeBg} transition-colors duration-1000`}>
      <div className="max-w-2xl mx-auto w-full relative">
        <Link href="/reading-room" className="absolute -top-12 left-0 text-sm text-brand-soft hover:text-white transition-colors">
          ← Back to Reading Room
        </Link>

        <header className="mb-12 border-b border-white/5 pb-12 text-center">
          <div className="flex justify-center gap-3 mb-6">
            ...
            {post.category && post.category !== "Uncategorized" && (
              <span className="text-[10px] uppercase tracking-widest text-brand-soft border border-white/10 px-3 py-1 rounded-full">
                {post.category}
              </span>
            )}
          </div>
          ...
          {post.title && (
            <h1 className="text-4xl md:text-5xl text-white mb-8 leading-snug font-serif">
              {post.title}
            </h1>
          )}
          ...
          <div className="flex items-center justify-center gap-3">
            <Link href={`/room/${username}`} className="flex items-center gap-2 hover:opacity-80 transition-opacity">
              {profile.avatar_url ? (
                ...
                <div className="w-8 h-8 rounded-full bg-black/50 border border-white/10 flex items-center justify-center text-xs font-sans text-brand-soft">
        ...
        {/* Content Render: we use prose for Tiptap HTML output */}
        <div 
          className="prose prose-invert prose-lg md:prose-xl max-w-none mb-16 leading-relaxed font-serif"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 justify-center">
              {post.attached_pins.map((pin: any, idx: number) => (
                <a key={idx} href={`/api/analytics/click?url=${encodeURIComponent(pin.url)}&profile_id=${profile.id}`} target="_blank" rel="noopener noreferrer" className="block w-full group">
                  <div className="bg-[#121212] border border-brand-accent/20 rounded-3xl overflow-hidden shadow-[0_0_30px_rgba(252,163,17,0.05)] hover:shadow-[0_0_40px_rgba(252,163,17,0.15)] hover:border-brand-accent/60 transition-all duration-700 relative h-full flex flex-col">
                    <div className="absolute inset-0 bg-gradient-to-b from-brand-accent/5 to-transparent pointer-events-none" />
                    <div className="bg-black/60 py-6 px-6 text-center border-b border-white/5 relative z-10 group-hover:bg-white/5 transition-colors">
                    ...
                    <div className="p-6 relative z-10 text-center flex-grow flex flex-col justify-between">
                      <div>
                        <h3 className="text-lg text-white mb-2 font-serif group-hover:text-brand-accent transition-colors line-clamp-2">
                          {pin.title}
                        </h3>
                      ...
                      <div className="inline-block bg-white text-black px-6 py-2 rounded-full text-[10px] uppercase tracking-widest font-bold group-hover:scale-105 transition-transform mx-auto">
                        View Details
                      </div>
                    </div>
                  </div>
